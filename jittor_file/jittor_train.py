...
import jittor as jt
from jittor_model import MyNet
from jittor_data import MyDataset
from jittor_myag import Args
...
class TrainInterface(object):
    ...
    def __train(model, train_loader, optimizer, epoch, num_train, opts):
        """
        完成一个epoch的训练
        :param model: jt.nn.Module, 需要训练的网络
    ...
    def __save_model(model, epoch, opts):
        ...
        jt.save(model.state_dict(), save_dir)
    ...
    def main(self):
        ...
        optimizer = jt.optim.SGD(model.parameters(), lr=opts.lr, momentum=0.95, weight_decay=opts.weight_decay)
