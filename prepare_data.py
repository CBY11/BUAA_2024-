...
def create_csv_txt(img_dir, anno_dir, save_root_dir, train_val_ratio=0.9, padding=10, debug=False):
    ...
    # ===============================================================================================================
    # 存处理好的图片的文件夹
    save_img_dir = os.path.join(os.path.join(anno_dir, "cby_yawn_forYolov1"), "img")
...
if __name__ == '__main__':
    ...
    img_dir = os.path.join(STATIC_DATASET_PATH, "yawn_101-200")  # 原始图像文件夹
    anno_dirs = [STATIC_DATASET_PATH]  # 标注文件
    save_dir = os.path.join(STATIC_DATASET_PATH, "cby_yawn_forYolov1")  # 保存处理后的数据(图片+标签)的文件夹
